...
class TrackerNode:
    def __init__(self):
        ...
        path = roslib.packages.get_pkg_dir("yolov8_segmentation_tracking_ros")
    ...
    def image_callback(self, msg):
        ...
        x_min = int(self.roi_normalized[0][0] * image_width)
        x_max = int(self.roi_normalized[1][0] * image_width)
        y_min = int(self.roi_normalized[0][1] * image_height)
        y_max = int(self.roi_normalized[2][1] * image_height)
        ...
        if results is not None:
            ...
            yolo_result_msg.detections = self.create_detections_array(results, x_min, y_min)
            yolo_result_image_msg = self.create_result_image(results, cv_image, x_min, y_min)
            if self.use_segmentation:
                yolo_result_msg.masks = self.create_segmentation_masks(results)
            self.results_pub.publish(yolo_result_msg)
            self.result_image_pub.publish(yolo_result_image_msg)

    def create_detections_array(self, results, x_offset, y_offset):
        ...
        for bbox, cls, conf in zip(bounding_box, classes, confidence_score):
            ...
            detection.bbox.center.x = float(bbox[0]) + x_offset
            detection.bbox.center.y = float(bbox[1]) + y_offset
    ...
    def create_result_image(self, results, original_image, x_offset, y_offset):
        ...
        result_image[y_offset:y_offset + plotted_image.shape[0], x_offset:x_offset + plotted_image.shape[1]] = plotted_image
